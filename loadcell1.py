...
doc_ref_FA_p1 = doc_ref.collection(u'featureAnalysis').document(u'period1')

# doc_ref_on_off = doc_ref_col_FA.document(u'on_off') # on_off값 가져오기

# doc_ref_on_FA_p2 = doc_ref_col_FA.document(u'period1') # on_off값 가져오기
...
try:
    while True :
        ...
        if distance < 40:
            ...
            # loadcell 센서 동작 serial 통신  # on_off는 일단 해제
            '''
            while True:
            ...
            if __name__ == '__main__':
                ...
                if data <= 0 :
                    ...
                    pre_weight = doc_ref.get().to_dict()['weight1']
                    doc_ref_FA_p1.update({
                        date.strftime("%Y%m%d") : pre_weight
                    })

                    # 빨래양과 주기 가져오기

                    dic_ideal = doc_ref_FA_p1.get().to_dict()
